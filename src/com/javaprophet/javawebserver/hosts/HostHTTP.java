...
import com.javaprophet.javawebserver.http.networking.ThreadAccept;
import com.javaprophet.javawebserver.http.networking.ThreadConnection;
import com.javaprophet.javawebserver.http.networking.ThreadWorker;
...
public class HostHTTP extends Host {
	...
	public void formatConfig(HashMap<String, Object> map) {
		...
		for (String vkey : vhosts.keySet()) {
			...
			if (!vhost.containsKey("htdocs")) vhost.put("htdocs", JavaWebServer.fileManager.getBaseFile("htdocs").toString());
			if (!vhost.containsKey("htsrc")) vhost.put("htsrc", JavaWebServer.fileManager.getBaseFile("htsrc").toString());
		}
		for (String vkey : vhosts.keySet()) {
