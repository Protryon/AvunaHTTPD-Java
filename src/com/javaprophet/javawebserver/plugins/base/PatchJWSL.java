...
public class PatchJWSL extends Patch {
	...
	public boolean shouldProcessResponse(ResponsePacket response, RequestPacket request, byte[] data) {
		return response.headers.hasHeader("Content-Type") && response.headers.getHeader("Content-Type").value.equals("application/x-jwsl") && response.body != null && data != null && data.length > 0;
	}
